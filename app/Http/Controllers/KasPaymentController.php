    ...
    public function processPayment(Request $request, Kas $kas)
    {
        ...
        try {
            ...
            if ($request->hasFile('proof_of_payment')) {
                $filePath = $request->file('proof_of_payment')->store('public/bukti_pembayaran');
                $filePath = str_replace('public/', 'storage/', $filePath);
            }

            // Normalize payment method using the Kas model method
            $metodeBayar = Kas::normalizePaymentMethod($request->payment_method);
            ...
            $kas->update([
                ...
                'jumlah_dibayar' => $request->amount_paid,
                'metode_bayar' => $metodeBayar,
                'bukti_bayar_file' => $filePath,
                'bukti_bayar_notes' => $request->notes,
            ...
            if ($status === 'lunas') {
                $kas->update([
                    ...
                    'confirmation_notes' => 'Pembayaran tunai langsung dikonfirmasi.',
                ]);
