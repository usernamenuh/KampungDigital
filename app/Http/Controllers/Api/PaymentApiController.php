...
use App\Models\Kk;
use Carbon\Carbon;
use Illuminate\Support\Facades\Storage; // Added for qr_code_url accessor
  ...
  public function getPaymentInfoByRt(Request $request, $rtId)
  {
      try {
          ...
          if ($user->role === 'rt') {
              ...
              if (!$penduduk || !$penduduk->kk || ($penduduk->kk->rt->id ?? null) != $rtId) { // Added null check for rt->id
                  return response()->json([
                      'success' => false,
                      'message' => 'Akses tidak diizinkan untuk RT ini'
                  ], 403);
          ...
          $data = [
              ...
              'qr_code_url' => $paymentInfo->qr_code_url, // Use accessor
