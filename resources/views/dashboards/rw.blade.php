...
function rwDashboardData() {
  return {
      ...
      bantuanPending: 0,
      activities: [],
      rtData: [], // This will hold data for RTs under this RW
      charts: {
          population: null
      },
          ...
          try {
              ...
              if (response.ok) {
                  ...
                  if (data.success) {
                      ...
                      this.rtData = data.data.rtData || []; // Populate rtData from backend
                      this.loadActivities(); // Load activities after data is loaded
      ...
      async loadActivities() {
          try {
              const response = await fetch('/api/dashboard/activities?limit=5', { // Corrected API route
                  method: 'GET',
                  headers: {
                      'X-Requested-With': 'XMLHttpRequest',
                      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content')
