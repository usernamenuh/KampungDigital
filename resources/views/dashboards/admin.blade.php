...
<div x-data="adminDashboardData()" x-init="initDashboard()" class="p-6 space-y-6">
  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-6 gap-4">
          <a href="{{ route('admin.users.index') }}" class="flex flex-col items-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors">
...
function adminDashboardData() {
  return {
      ...
      async loadActivities() {
          try {
              const response = await fetch('/ajax/dashboard/activities?limit=20', {
                  method: 'GET',
                  headers: {
                      'X-Requested-With': 'XMLHttpRequest',
                      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
      ...
      async loadSystemMonitoring() {
          try {
              const response = await fetch('/ajax/dashboard/system-monitoring', {
                  method: 'GET',
                  headers: {
                      'X-Requested-With': 'XMLHttpRequest',
                      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
      ...
      async clearCache() {
          try {
              const response = await fetch('/ajax/dashboard/clear-cache', { 
                  method: 'POST',
                  headers: {
                      'X-Requested-With': 'XMLHttpRequest',
                      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
      ...
      async checkSystemHealth() {
          try {
              const response = await fetch('/ajax/dashboard/system-health', {
                  method: 'GET',
                  headers: {
                      'X-Requested-With': 'XMLHttpRequest',
                      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
