...
@section('page-description', 'Selamat datang, ' . auth()->user()->name . '! Kelola sistem dan data.')

@section('content')
<div x-data="adminDashboardData()" x-init="initDashboard()" class="p-6 space-y-6">
    <!-- Admin Info Header -->
    <div class="p-6 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 transition-all hover:shadow-lg bg-gradient-to-r from-green-500 to-teal-600 text-white">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold">Selamat datang, Admin</h2>
                <p class="text-green-100">Kelola sistem dan data aplikasi</p>
            </div>
            <div class="text-right">
                <p class="text-sm text-green-100" x-text="currentDate"></p>
                <div class="flex items-center justify-end mt-2">
                    <div :class="isOnline ? 'bg-green-400 animate-pulse' : 'bg-red-400'" class="w-2 h-2 rounded-full mr-2"></div>
                    <span class="text-sm text-green-100" x-text="isOnline ? 'Online' : 'Offline'">Online</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 transition-all hover:shadow-md">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-800 dark:text-white mt-2" x-text="totalUsers">0</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 transition-all hover:shadow-md">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="flex items-center mt-2">
                        <span class="text-sm text-purple-600 font-medium">Terdaftar</span>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 transition-all hover:shadow-md">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="flex items-center mt-2">
                        <span class="text-sm text-orange-600 font-medium">Terdaftar</span>
                    </div>
                </div>
                <div class="p-3 bg-orange-100 dark:bg-orange-900 rounded-xl">
                    <i data-lucide="map" class="w-8 h-8 text-orange-600 dark:text-orange-400"></i>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Kesehatan Sistem</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Status komponen aplikasi.</p>
                </div>
                <button @click="loadSystemHealth()" class="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                    <i data-lucide="refresh-cw" class="w-4 h-4 text-gray-500"></i>
                </button>
            </div>
            
            <div class="space-y-4">
                <template x-for="(status, component) in systemHealth.checks" :key="component">
                    <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <div class="flex items-center space-x-3">
                            <div :class="{
                                'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-400': status === 'ok',
                                'bg-red-100 text-red-600 dark:bg-red-900 dark:text-red-400': status === 'error'
                            }" class="p-2 rounded-full">
                                <i :data-lucide="status === 'ok' ? 'check' : 'x'" class="w-4 h-4"></i>
                            </div>
                            <span class="text-sm font-medium text-gray-800 dark:text-white" x-text="component.replace('_', ' ').toUpperCase()"></span>
                        </div>
                        <span :class="{
                            'text-green-600 dark:text-green-400': status === 'ok',
                            'text-red-600 dark:text-red-400': status === 'error'
                        }" class="text-sm font-semibold" x-text="status.toUpperCase()"></span>
                    </div>
                </template>
                <div x-show="systemHealth.status === 'degraded'" class="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg text-sm text-yellow-800 dark:text-yellow-200">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <a href="{{ route('users.index') }}" class="flex flex-col items-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors">
                ...
                <span class="text-sm font-medium text-blue-600">Kelola Pengguna</span>
            </a>
            <a href="{{ route('rt-rw.index') }}" class="flex flex-col items-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors">
                <i data-lucide="map" class="w-8 h-8 text-green-600 mb-2"></i>
                <span class="text-sm font-medium text-green-600">Kelola RT/RW</span>
            </a>
            ...
            <button @click="clearApplicationCache()" class="flex flex-col items-center p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg hover:bg-orange-100 dark:hover:bg-orange-900/30 transition-colors">
                <i data-lucide="refresh-ccw" class="w-8 h-8 text-orange-600 mb-2"></i>
                <span class="text-sm font-medium text-orange-600">Bersihkan Cache</span>
            </button>
        </div>
    </div>
</div>
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.success) {
                        ...
                        this.isOnline = true;
                        this.connectionStatus = 'online';
                        console.log('✅ Admin data loaded successfully:', data.data);
                    } else {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.success) {
                        ...
                        console.log('✅ System health loaded successfully:', data);
                    } else {
                        throw new Error(data.message || 'Gagal memuat kesehatan sistem');
                    }
                } else {
                    throw new Error('Response tidak OK: ' + response.status);
                }
                ...
                this.systemHealth = { status: 'unhealthy', checks: { database: 'error', cache: 'error', storage: 'error' } };
                this.showConnectionError('Gagal memuat kesehatan sistem');
            }
        },
        ...
        async clearApplicationCache() {
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    if (data.success) {
                        this.showSuccessMessage('Cache aplikasi berhasil dibersihkan!');
                        this.loadAllData(); // Refresh data after clearing cache
                    } else {
                        throw new Error(data.message || 'Gagal membersihkan cache');
                    }
                } else {
                    throw new Error('Response tidak OK: ' + response.status);
                }
                ...
                this.showConnectionError('Gagal membersihkan cache');
            }
        },
