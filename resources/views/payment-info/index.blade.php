...
@section('content')
{{-- Menyesuaikan kelas div utama agar lebih sesuai dengan layout aplikasi --}}
<div x-data="paymentInfoData()" x-init="init()" class="w-full px-4 py-6 sm:px-6 lg:px-8">
{{-- Notification Banner --}}
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 sm:p-8">
  ...
  <div x-show="paymentInfos.length > 0" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-2 items-stretch">
      <template x-for="info in paymentInfos" :key="info.id">
          <div class="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-600 flex flex-col h-full">
              <div class="flex items-center justify-between mb-4">
                  <h4 class="font-bold text-[15px] text-gray-800 dark:text-white" x-text="'Informasi Pembayaran RT ' + info.rt_no + (info.rw_no !== 'N/A' ? ' / RW ' + info.rw_no : '')"></h4>
                  <div class="flex items-center space-x-2">
                      <span :class="info.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-600 dark:text-gray-300'" class="px-3 py-1 rounded-full text-[13px] font-semibold">
              ...
              <div class="grid grid-cols-1 gap-4 text-sm text-gray-600 dark:text-gray-400 flex-grow">
                  ...
                  <div class="bg-white dark:bg-gray-800 p-4 rounded-md border border-gray-100 dark:border-gray-600 shadow-sm">
                      ...
                      <template x-if="!info.has_bank_transfer">
                          <p class="text-gray-500 dark:text-gray-400 italic">Tidak ada informasi transfer bank.</p>
                      </template>
                  </div>
                  ...
                  <div class="bg-white dark:bg-gray-800 p-4 rounded-md border border-gray-100 dark:border-gray-600 shadow-sm">
                      ...
                      <template x-if="!info.has_e_wallet">
                          <p class="text-gray-500 dark:text-gray-400 italic">Tidak ada informasi E-Wallet.</p>
                      </template>
                  </div>
                  ...
                  <div class="bg-white dark:bg-gray-800 p-4 rounded-md border border-gray-100 dark:border-gray-600 shadow-sm flex flex-col items-center">
                      ...
                      <template x-if="info.qr_code_url">
                          <img :src="info.qr_code_url" alt="QR Code" class="w-32 h-32 object-contain rounded-md p-1 mb-2">
                      </template>
                      <template x-if="!info.qr_code_url">
                          <div class="w-32 h-32 flex items-center justify-center bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500 rounded-md border border-gray-200 dark:border-gray-600 p-1 mb-2">
                      ...
                      <template x-if="!info.qr_code_url && !info.qr_code_account_name && !info.qr_code_description">
                          <p class="text-gray-500 dark:text-gray-400 italic">Tidak ada informasi QR Code.</p>
                      </template>
                  </div>
                  ...
                  <div class="bg-white dark:bg-gray-800 p-4 rounded-md border border-gray-100 dark:border-gray-600 shadow-sm">
                      ...
                      <template x-if="!info.payment_notes">
                          <p class="text-gray-500 dark:text-gray-400 italic">Tidak ada catatan pembayaran umum.</p>
                      </template>
                  </div>
              </div>
          </div>
...
<div x-show="isModalOpen" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 sm:p-6" x-cloak>
  <div @click.away="closeModal()" class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[95vh] overflow-y-auto transform transition-all duration-300 ease-out scale-95 sm:scale-100 opacity-0"
      ...
      <form @submit.prevent="submitForm()" class="p-6 sm:p-8 space-y-6">
          ...
          <!-- Bank Transfer Section (Now single) -->
          <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 mb-4 border border-gray-200 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 relative">
                  <div>
                      <label for="bank_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama Bank</label>
                      <select x-model="form.bank_name" id="bank_name"
                              class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-900 dark:text-white">
                  ...
                  <div>
                      ...
                      <input type="text" x-model="form.bank_account_number" id="bank_account_number"
                  ...
                  <div class="md:col-span-2">
                      ...
                      <input type="text" x-model="form.bank_account_name" id="bank_account_name"
                             class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500">
              ...
              <div class="space-y-4">
                  ...
                  <div x-show="form.qr_code_url || qrCodePreview" class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 p-3 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800">
                      ...
                      <template x-if="!qrCodePreview && form.qr_code_url">
                          <img :src="form.qr_code_url" alt="Current QR Code" class="w-32 h-32 object-contain rounded-md p-1 mb-2">
                      </template>
...
async fetchPaymentInfos() {
    try {
        ...
        if (data.success) {
            ...
            this.rts = data.rts_for_selection || [];
...
resetForm() {
    this.form = {
        rt_id: '',
        bank_name: '', // Reset to empty
        bank_account_number: '', // Reset to empty
        bank_account_name: '', // Reset to empty
        dana_number: '',
        dana_account_name: '',
        gopay_number: '',
        gopay_account_name: '',
...
openModalForEdit(info) {
    ...
    console.log('Editing Payment Info with ID:', this.currentPaymentInfoId);
    this.form = {
        ...info,
        rt_id: info.rt_id,
        bank_name: info.bank_name || '', // Populate single bank fields
...
removeQrCode() {
    this.form.qr_code_file = null;
    this.form.qr_code_url = null;
    this.qrCodePreview = null;
    this.form.clear_qr_code = true;
    const qrCodeFileInput = document.getElementById('qr_code_file');
    if (qrCodeFileInput) {
        qrCodeFileInput.value = '';
    }
...
async submitForm() {
    ...
    // Append all form fields, including empty ones, so they can be cleared on the backend
    for (const key in this.form) {
        if (key === 'qr_code_file') {
            if (this.form[key]) { // Only append if a file is selected
                formData.append(key, this.form[key]);
            }
        } else if (key === 'qr_code_url') {
            // Skip this, it's a frontend-only accessor
            continue;
        } else if (key === 'is_active' || key === 'clear_qr_code') {
            // Convert boolean to 0 or 1 for backend
            formData.append(key, this.form[key] ? 1 : 0);
        } else {
            // Append all other fields, including empty strings, to ensure they are sent
            formData.append(key, this.form[key] || ''); 
        }
    }
    ...
    console.log('Submitting form to URL:', url, 'with method:', method);
    ...
    try {
        ...
        if (data.success) {
            ...
            this.closeModal();
            await this.fetchPaymentInfos(); // Re-fetch to update UI
        } else {
            if (data.errors) {
